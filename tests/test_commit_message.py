...
class CommitMessageTests(unittest.TestCase):
    def test_accepts_scoped_and_unscoped_conventional_commits(self):
        self.assertIsNone(validate_header("fix(lidarr): preserve downloaded release selection"))
        self.assertIsNone(validate_header("docs: explain local configuration"))

    def test_rejects_unknown_types_and_scopes(self):
        self.assertIn("Invalid commit type", validate_header("change: update behavior"))
