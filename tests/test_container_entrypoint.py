...
from music_importer.container_entrypoint import (
    APPLICATION_GID,
    APPLICATION_UID,
    prepare_storage,
    run,
...
class ContainerEntrypointTests(unittest.TestCase):
    def test_prepare_storage_creates_and_owns_mount_roots(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("music_importer.container_entrypoint.os.chown") as chown:
                prepare_storage((storage,))

            self.assertTrue(storage.exists())
            chown.assert_called_once_with(storage, APPLICATION_UID, APPLICATION_GID)

    @patch("music_importer.container_entrypoint.os.execvp")
    @patch("music_importer.container_entrypoint.os.setuid")
    @patch("music_importer.container_entrypoint.os.setgid")
    @patch("music_importer.container_entrypoint.os.setgroups")
    @patch("music_importer.container_entrypoint.prepare_storage")
    @patch("music_importer.container_entrypoint.os.geteuid", return_value=0)
    ...
    def test_run_does_not_prepare_storage_when_already_unprivileged(self, prepare, execvp):
        with patch("music_importer.container_entrypoint.os.geteuid", return_value=1000):
            run(("server",))
