...
import {
  openDatabase,
  SCHEMA_VERSION,
} from "../src/server/persistence/database";
...
describe("SQLite schema", () => {
  it("creates the compatible schema-v8 database", () => {
    ...
    expect(
      database
        .prepare(
          "SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'local_playlist_additions'",
        )
        .get(),
    ).toBeDefined();
    database.close();
  ...
  it("marks unfinished jobs interrupted when opening an existing schema-v8 database", () => {
    ...
    expect(
      reopened
        .prepare("SELECT status FROM jobs WHERE id = 'job-1'")
        .pluck()
        .get(),
    ).toBe("interrupted");
    reopened.close();
