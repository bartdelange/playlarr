...
it("claims queued work once in creation order", () => { const directory = mkdtempSync(path.join(tmpdir(), "playlarr-jobs-")); paths.push(directory); const jobs = new JobRepository(openDatabase(path.join(directory, "music-importer.db"))); const first = jobs.create("first"); jobs.create("second"); expect(jobs.claimNext()?.id).toBe(first.id); expect(jobs.claimNext()?.kind).toBe("second"); expect(jobs.claimNext()).toBeUndefined(); });
