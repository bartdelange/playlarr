...
describe("import repository", () => {
  ...
  it("applies an update transactionally, retains a matched occurrence, and supersedes approved plans", () => { const imports = repository(); const imported = imports.createImport({ source: "spotify", id: "mix", name: "Old" }); imports.replaceTracks(imported.id, [{ source: "spotify", sourceTrackId: "keep", title: "Keep", artists: ["Artist"], album: "Album" }, { source: "spotify", sourceTrackId: "remove", title: "Remove", artists: ["Artist"], album: "Album" }]); const kept = imports.entries(imported.id)[0]; const database = (imports as unknown as { database: import("better-sqlite3").Database }).database; database.prepare("INSERT INTO lidarr_plans (id, import_id, status, created_at) VALUES ('approved', ?, 'approved', ?)").run(imported.id, new Date().toISOString()); const update = imports.applyPlaylistUpdate(imported.id, { source: "spotify", id: "mix", name: "New" }, [{ position: 0, track: { source: "spotify", sourceTrackId: "new", title: "New", artists: [], album: "" } }, { position: 1, track: { source: "spotify", sourceTrackId: "keep", title: "Keep", artists: ["Artist"], album: "Album" } }]); expect(update).toMatchObject({ added: 1, removed: 1 }); expect(imports.entries(imported.id)[1].id).toBe(kept.id); expect(database.prepare("SELECT status FROM lidarr_plans WHERE id = 'approved'").pluck().get()).toBe("superseded"); });
});
