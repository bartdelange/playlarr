...
export async function login(form: FormData) {
  ...
  if (!security.allowLogin(client))
    redirect(
      "/login?error=Too%20many%20login%20attempts%3B%20try%20again%20later",
    );
...
export async function changePassword(form: FormData) {
  await requireCsrf(form);
  if (
    !(await security.verifyPassword(String(form.get("current_password") ?? "")))
  )
