    ...
    <main>
      <section className="hero">
        <div>
          ...
          <h1>Playlists</h1>
          <p>Continue matching, approve Lidarr releases, and generate synchronized playlists.</p>
        </div>
            ...
            <div className="playlist-home-list">
              {rows.map((row) => (
                <article className="playlist-home-row" key={row.imported.id}>
                  <Link className="playlist-home-main" href={`/imports/${row.imported.id}`}>
                    <div>
                      <span className="badge">{row.imported.source}</span>
                      <h2>{row.imported.playlistName}</h2>
                      <small>Updated {row.imported.updatedAt.slice(0, 16).replace("T", " ")}</small>
                    </div>
                    ...
                    <div className="playlist-next">
                      <span className="badge">{row.imported.workflowState.replaceAll("_", " ")}</span>
                      <strong>{row.nextAction}</strong>
                    </div>
                  </Link>
                  {row.job && (
                    <Link className="playlist-job" href={`/jobs/${row.job.id}`}>
                      <span className={`badge job-${row.job.status}`}>{row.job.status}</span>
                      <small>
                        {row.job.kind.replaceAll("_", " ")} · {row.job.current} / {row.job.total}
                      </small>
                    </Link>
                  )}
                </article>
              ...
              {jobs.map((job) => (
                <Link href={`/jobs/${job.id}`} key={job.id}>
                  <div>
                    ...
                    <small>{job.currentItem ?? `${job.current} / ${job.total}`}</small>
                  </div>
