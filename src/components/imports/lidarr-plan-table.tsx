...
const actionGuide = [
  ["unchanged", "No change", "The selected release is already represented and configured in Lidarr."],
  [
  ...
  ["create_release", "Create release", "Add this specific MusicBrainz album, EP, or single to Lidarr."],
  ["monitor_artist", "Monitor artist", "Monitor the artist while keeping new-item monitoring disabled."],
  [
    "monitor_release",
    "Monitor release",
    "Turn monitoring on only when a requested recording is missing and must be acquired.",
  ],
  ["queue_search", "Queue search", "Ask Lidarr to search its indexers for the selected release."],
  ["skip", "Skip", "Make no Lidarr change because the binding is unresolved or excluded by a safety rule."],
] as const;
...
export function LidarrPlanTable({
  ...
  const columnLabels = ["#", "Status", "Source track", "Lidarr album / release", "Plan", "Actions"];
    ...
    () =>
      rows.filter((row) => {
        const search = [
          ...
          ...row.releases.flatMap((release) => [release.artistName, release.title]),
        ]
          .join(" ")
          .toLocaleLowerCase();
        const actionMatches =
    ...
    <>
      ...
      <div className="mapping-filters">
        ...
        >
          ...
          <option value="reuse_downloaded_release">Reuse downloaded release</option>
          ...
          <tbody>
            {visible.map((row) => (
              <tr key={row.entry.id}>
                ...
                {columns[1] && (
                  <td>
                    <span className="badge">{row.entry.resolutionState.replaceAll("_", " ")}</span>
                  </td>
                )}
                {columns[2] && (
                  <td>
                    ...
                    {row.releases.length ? (
                      row.releases.map((release) => (
                        <div className="release-link" key={release.sourceGroup}>
                          ...
                          <small>
                            {release.artistName || row.entry.track.artists.join(", ") || "Unknown artist"} ·{" "}
                            {release.title || row.entry.track.album || "Lidarr release"}
                          </small>
                          {release.matchedTrack && (
                            <div className="matched-lidarr-track">
                              ...
                              <small>
                                ...
                                {release.matchedTrack.has_file ? "File downloaded" : "No file"}
                          ...
                          {release.sourceGroup !== release.lidarrGroup && (
                            <small>Rebound because this Lidarr release contains the selected track.</small>
                          )}
                        </div>
                      ))
                    ) : (
                  ...
                  <td>
                    {row.actions.length ? (
                      row.actions.map((action, index) => (
                        <div className="plan-decision" key={`${action.action}-${index}`}>
                          <span className={`badge plan-${action.action}`}>{action.action.replaceAll("_", " ")}</span>
                          {row.artistActions.includes(action) && (
                            <small className="action-scope">
                              Artist-level action · shared by this artist&apos;s requested songs
                            </small>
                          )}
                          <small>{reasons[action.reason ?? ""] ?? action.reason?.replaceAll("_", " ")}</small>
                        </div>
                      ))
                    ) : row.entry.result.resolvedVia && row.releases.length ? (
                      <>
                        <span className="badge plan-unchanged">No explicit action</span>
                        <small>Rebuild this older plan for an explicit unchanged outcome.</small>
                      </>
                    ) : (
                      <>
                ...
                {columns[5] && (
                  <td className="binding-actions">
                    {editable && (
                      <Link href={`/entries/${row.entry.id}/review?plan_id=${planId}`}>
                        {row.entry.result.resolvedVia ? "Change track" : "Resolve track"}
                      </Link>
                    )}
                    {row.variousArtistsOverride ? (
                      <span className="badge plan-monitor_release">VA safety override queued</span>
                    ) : (
                    ...
                    {editable && (row.entry.result.releaseGroupIds?.length ?? 0) > 1 && (
