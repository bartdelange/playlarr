...
export class LibraryRepository {
  ...
  saveStatus(importId: string, statuses: LibraryStatus[]): void {
    const entries = new Map(
      (
        this.database.prepare("SELECT id, position FROM playlist_entries WHERE import_id = ?").all(importId) as {
          id: number;
          position: number;
        }[]
    ...
    const persist = this.database.transaction(() => {
      for (const status of statuses) {
        ...
        if (!entry) throw new Error(`unknown playlist position: ${status.position}`);
        this.database
          .prepare(
  ...
  recordExport(importId: string, outputPath: string, written: number, missing: number): string {
    const id = randomUUID();
    this.database
      .prepare(
    ...
    this.database
      .prepare("UPDATE imports SET workflow_state = 'playlist_generated', updated_at = ? WHERE id = ?")
      .run(now(), importId);
    return id;
  }
  ...
  savePlaylistAnalysis(
    ...
    this.database
      ...
      .run(source, playlistId, playlistName, status, JSON.stringify(result), now());
  }
  playlistAnalyses(source: string): Record<string, PlaylistAnalysis> {
    const rows = this.database
      .prepare("SELECT playlist_id, status, result_json, updated_at FROM playlist_analyses WHERE source = ?")
      .all(source) as {
      playlist_id: string;
      status: string;
      result_json: string;
