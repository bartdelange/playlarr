...
export class LidarrPlanRepository {
  ...
  save(importId: string, plan: LidarrPlan): string {
    ...
    this.database.transaction(() => {
      ...
      plan.actions.forEach((action, position) =>
        insert.run(id, position, JSON.stringify(action)),
      );
      this.database
        .prepare(
  ...
  get(id: string): { importId: string; status: string; plan: LidarrPlan } {
    const header = this.database
      .prepare("SELECT import_id, status FROM lidarr_plans WHERE id = ?")
      .get(id) as { import_id: string; status: string } | undefined;
  ...
  approve(id: string): void {
    const outcome = this.database
      .prepare(
        "UPDATE lidarr_plans SET status = 'approved', approved_at = ? WHERE id = ? AND status = 'draft'",
      )
      .run(now(), id);
    if (outcome.changes !== 1)
      throw new Error("only a current draft plan can be approved");
  }
  recordExecution(
    id: string,
    results: { outcome: string; details?: string }[],
  ): void {
    const plan = this.get(id);
    if (plan.status !== "approved")
      throw new Error("only an approved plan can record execution");
    this.database.transaction(() => {
      const insert = this.database.prepare(
        "INSERT INTO lidarr_execution_results (plan_id, action_position, attempted_at, outcome, details) VALUES (?, ?, ?, ?, ?)",
      );
      results.forEach((result, position) =>
        insert.run(id, position, now(), result.outcome, result.details ?? ""),
      );
      const failed = results.some((result) => result.outcome === "failed");
      this.database
        .prepare("UPDATE lidarr_plans SET status = ? WHERE id = ?")
        .run(failed ? "failed" : "completed", id);
      this.database
        .prepare(
          "UPDATE imports SET workflow_state = ?, updated_at = ? WHERE id = ?",
        )
        .run(
          failed ? "execution_failed" : "waiting_for_downloads",
          now(),
          plan.importId,
        );
    })();
