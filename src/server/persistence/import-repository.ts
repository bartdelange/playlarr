...
export class ImportRepository {
  ...
  createImport(playlist: PlaylistInfo, metadata: object = {}, importId = randomUUID()): StoredImport {
    ...
    this.database
      .prepare(
        `INSERT INTO imports (id, source, source_playlist_id, playlist_name, playlist_path, playlist_metadata_json, workflow_state, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, 'acquiring', ?, ?)`,
      )
      .run(
        importId,
        playlist.source,
  ...
  getImport(importId: string): StoredImport {
    ...
    return {
      ...
      sourcePlaylistId: row.source_playlist_id as string,
      playlistName: row.playlist_name as string,
      playlistPath: optional(row.playlist_path),
  ...
  entries(importId: string): StoredEntry[] {
    return (
      ...
      track: {
        source: row.import_source as string,
        sourceTrackId: row.source_track_id as string,
        title: row.title as string,
        artists: JSON.parse(row.artists_json as string) as string[],
  ...
  replaceTracks(importId: string, tracks: SourceTrack[]): void {
    this.replaceAcquiredTracks(
      importId,
      tracks.map((track, position) => ({ position, track })),
    );
  }
  ...
  replaceAcquiredTracks(importId: string, entries: AcquiredTrack[]): void {
    const replace = this.database.transaction(() => {
      ...
      const resolve = this.database.prepare(
        "INSERT INTO resolutions (entry_id, state, method, result_json, evidence_json, updated_at) VALUES (?, ?, ?, '{}', ?, ?)",
      );
      for (const entry of entries) {
        const result = add.run(
          ...
          entry.track.title,
          JSON.stringify(entry.track.artists),
          entry.track.album,
          entry.track.isrc ?? null,
          entry.track.durationMs ?? null,
          entry.skipReason ? "skipped" : "acquired",
  ...
  applyPlaylistUpdate(importId: string, playlist: PlaylistInfo, current: AcquiredTrack[]): PlaylistUpdate {
    ...
    for (const field of ["sourceTrackId", "isrc"] as const) {
      ...
      current.forEach((entry, index) => {
        ...
        const value = entry.track[field];
        const old = value ? buckets.get(value)?.shift() : undefined;
        if (old) {
    ...
    this.database.transaction(() => {
      ...
      const change = this.database.prepare(
        "UPDATE playlist_entries SET position = ?, source_track_id = ?, title = ?, artists_json = ?, album = ?, isrc = ?, duration_ms = ?, acquisition_status = ?, skip_reason = ? WHERE id = ?",
      );
      current.forEach((entry, index) => {
        ...
        if (old) change.run(...values, old.id);
        else {
          const inserted = add.run(importId, ...values);
          resolution.run(
