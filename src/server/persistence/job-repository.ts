...
export class JobRepository {
  ...
  list(limit = 50): StoredJob[] {
    return (
      this.database
        .prepare(
          "SELECT id FROM jobs ORDER BY CASE status WHEN 'running' THEN 0 WHEN 'queued' THEN 1 ELSE 2 END, CASE WHEN status = 'queued' THEN created_at END ASC, updated_at DESC LIMIT ?",
        )
  ...
  requestCancel(id: string): void {
    this.database
      .prepare(
        "UPDATE jobs SET cancel_requested = 1, status = CASE WHEN status = 'queued' THEN 'cancelled' ELSE status END, current_item = CASE WHEN status = 'running' THEN 'Cancellation requested' ELSE current_item END, updated_at = ? WHERE id = ?",
      )
      .run(now(), id);
  }
  ...
  claimNext(): StoredJob | undefined {
    return this.database.transaction(() => {
      ...
      const claimed = this.database
        .prepare(
          "UPDATE jobs SET status = 'running', updated_at = ? WHERE id = ? AND status = 'queued' AND cancel_requested = 0",
        )
        .run(now(), row.id);
      return claimed.changes === 1 ? this.get(row.id) : undefined;
    })();
  }
}
