...
export function openDatabase(databasePath: string): Database.Database {
  ...
  migrate(database);
  database.pragma("foreign_keys = ON");
  database.prepare("UPDATE jobs SET status = 'interrupted', updated_at = ? WHERE status = 'running'")
...
export function migrate(database: Database.Database): void {
  ...
  if (version < 2) migration(2, "CREATE TABLE resolution_candidates (id INTEGER PRIMARY KEY AUTOINCREMENT, entry_id INTEGER NOT NULL REFERENCES playlist_entries(id) ON DELETE CASCADE, position INTEGER NOT NULL, candidate_json TEXT NOT NULL, created_at TEXT NOT NULL, UNIQUE(entry_id, position));");
  if (version < 3) migration(3, "CREATE TABLE library_status (entry_id INTEGER PRIMARY KEY REFERENCES playlist_entries(id) ON DELETE CASCADE, classification TEXT NOT NULL, file_path TEXT, refreshed_at TEXT NOT NULL);");
  if (version < 4) migration(4, "CREATE TABLE playlist_analyses (source TEXT NOT NULL, playlist_id TEXT NOT NULL, playlist_name TEXT NOT NULL, status TEXT NOT NULL, result_json TEXT NOT NULL, updated_at TEXT NOT NULL, PRIMARY KEY(source, playlist_id));");
  if (version < 5) migration(5, "CREATE TABLE playlist_revisions (id TEXT PRIMARY KEY, import_id TEXT NOT NULL REFERENCES imports(id) ON DELETE CASCADE, created_at TEXT NOT NULL, before_json TEXT NOT NULL, after_json TEXT NOT NULL, added INTEGER NOT NULL, removed INTEGER NOT NULL, moved INTEGER NOT NULL, unchanged INTEGER NOT NULL); CREATE INDEX playlist_revisions_import ON playlist_revisions(import_id, created_at DESC);");
