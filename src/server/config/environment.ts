...
export function loadConfig(environment: Environment = process.env): AppConfig {
  ...
  return {
    ...
    tidal: {
      sessionFile: environment.TIDAL_SESSION_FILE ?? ".secrets/tidal-session.json",
    },
    spotify: {
      clientId: optional(environment.SPOTIFY_CLIENT_ID),
      redirectUri: environment.SPOTIFY_REDIRECT_URI ?? "http://127.0.0.1:8787/callback",
      tokenCache: environment.SPOTIFY_TOKEN_CACHE ?? ".secrets/spotify-token.json",
    },
    lidarr: {
