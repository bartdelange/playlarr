...
export function productionJobHandlers(database: Database.Database, config: AppConfig, settings: SettingsRepository): Record<string, JobHandler> {
  ...
  const resolution: JobHandler = async (job, progress, cancelled) => {
    ...
    const imported = imports.getImport(job.importId); const playlist = { source: imported.source, id: imported.sourcePlaylistId, name: imported.playlistName, path: imported.playlistPath }; const rows = entries.map((entry) => mappingRow(playlist, entry.track, resolutions.get(entry.id).result)); await writeMappingReports(config.outputDir, playlist, rows);
  };
  ...
  const lidarrPlanning: JobHandler = async (job, progress) => { if (!job.importId) throw new Error("Lidarr planning job has no import"); const rows = database.prepare("SELECT r.result_json, r.evidence_json, r.selected_release_group_id FROM resolutions r JOIN playlist_entries e ON e.id = r.entry_id WHERE e.import_id = ? ORDER BY e.position").all(job.importId) as { result_json: string; evidence_json: string; selected_release_group_id: string | null }[]; const results = rows.map((row) => { const result = JSON.parse(row.result_json) as import("../domain/musicbrainz").MusicBrainzResult; return row.selected_release_group_id ? { ...result, releaseGroupIds: [row.selected_release_group_id] } : result; }); const client = new LidarrClient({ url: value("lidarr_url", config.lidarr.url ?? ""), apiKey: value("lidarr_api_key", config.lidarr.apiKey ?? ""), rootFolder: value("lidarr_root_folder", config.lidarr.rootFolder), qualityProfileId: value("lidarr_quality_profile_id", config.lidarr.qualityProfileId), metadataProfileId: value("lidarr_metadata_profile_id", config.lidarr.metadataProfileId) }); progress(0, 2, "Loading Lidarr artists"); const artists = new Set((await client.artists()).map((artist) => String(artist.foreignArtistId))); const groups = new Set<string>(); for (const group of new Set(results.flatMap((result) => result.releaseGroupIds ?? []))) if ((await client.albumsByForeignId(group)).some((album) => album.foreignAlbumId === group)) groups.add(group); progress(1, 2, "Building read-only plan"); const allowed = new Set(rows.flatMap((row, index) => (JSON.parse(row.evidence_json) as Record<string, unknown>).allow_various_artists_release ? results[index].recordingIds ?? [] : [])); new LidarrPlanRepository(database).save(job.importId, planLidarr(results, artists, groups, allowed)); progress(2, 2, "Plan ready for approval"); };
  ...
  const updatePreview: JobHandler = async (job, progress, cancelled) => {
    ...
    const source = imported.source === "spotify" ? spotifyProvider(config, settings).source : imported.source === "tidal" ? tidalProvider(config, settings).source : undefined;
    ...
    if (cancelled()) return;
    new JobRepository(database).setPayload(job.id, { playlist, entries, snapshotToken: playlistSnapshotToken(entries) });
  ...
  const updatePlaylist: JobHandler = async (job, progress) => {
    ...
    if (preview.importId !== job.importId || preview.kind !== "playlist_update_preview" || preview.status !== "completed" || !preview.payload) throw new Error("playlist update preview is not ready");
    ...
    if (!approvedToken || approvedToken !== currentToken || preview.payload.snapshotToken !== currentToken) throw new Error("the source playlist changed; preview the update again");
    ...
    if (update.added || update.removed || update.updated || update.moved) imports.applyPlaylistUpdate(job.importId, playlist, entries);
    new JobRepository(database).setPayload(job.id, { previewJob: previewId, snapshotToken: currentToken, update });
    progress(2, 2, "Playlist update complete");
  };
  const generation: JobHandler = async (job, progress) => { if (!job.importId) throw new Error("playlist generation job has no import"); const imports = new ImportRepository(database); const imported = imports.getImport(job.importId); const entries = imports.entries(job.importId); const rows = database.prepare("SELECT e.position, r.result_json, l.file_path FROM playlist_entries e JOIN resolutions r ON r.entry_id = e.id LEFT JOIN library_status l ON l.entry_id = e.id WHERE e.import_id = ? ORDER BY e.position").all(job.importId) as { position: number; result_json: string; file_path: string | null }[]; const mappings = value<[string, string][]>("path_mappings", [["/music", "/music"]]); progress(0, 2, "Building ordered playlist"); let exported = buildPlaylistExport(entries.map((entry) => entry.track), rows.map((row) => JSON.parse(row.result_json)), new Map(rows.filter((row) => row.file_path).map((row) => [row.position, row.file_path!])), mappings); const additions = new LocalAdditionsRepository(database).list(job.importId); if (additions.length) { const navidrome = new NavidromeClient({ url: value("navidrome_url", config.navidrome.url ?? ""), username: value("navidrome_username", config.navidrome.username ?? ""), password: value("navidrome_password", config.navidrome.password ?? "") }); exported = appendLocalAdditions(exported, additions, await navidrome.paths(additions.map((addition) => addition.providerTrackId)), mappings, entries.length); } const output = playlistOutputPath(config.outputDir, imported.playlistName); await writeM3u(output, exported); new LibraryRepository(database).recordExport(job.importId, output, exported.entries.length, exported.missing.length); progress(2, 2, `Wrote ${output}`); };
  return { playlist_acquisition: acquisition, playlist_update_preview: updatePreview, playlist_update: updatePlaylist, resolution, resolution_retry: resolution, lidarr_planning: lidarrPlanning, lidarr_execution: lidarrExecution, playlist_generation: generation };
}
