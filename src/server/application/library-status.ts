...
const trackPath = (
  ...
  return file.relativePath && artistPath
    ? path.join(artistPath, String(file.relativePath))
    : undefined;
};
const matches = (track: Record<string, unknown>, result: MusicBrainzResult) =>
  result.recordingIds?.includes(
    String(track.foreignRecordingId ?? track.foreignTrackId ?? ""),
  ) ||
  Boolean(
...
export async function refreshLibraryStatus(
  ...
  for (const [position, result] of results.entries()) {
    ...
    const artist = result.primaryArtistId
      ? artists.get(result.primaryArtistId)
      : undefined;
    if (artist) {
      ...
      const direct = artistTracks.find(
      ...
      if (direct) {
        filePath = trackPath(
          direct,
          files,
          artist.path ? String(artist.path) : undefined,
        );
      ...
      if (!filePath)
        for (const group of result.releaseGroupIds ?? []) {
          ...
          const candidate = (
            await client.tracksByAlbumId(Number(album.id))
          ).find((track) => track.hasFile && matches(track, result));
          if (candidate) {
            filePath = trackPath(
              candidate,
              ownerFiles,
              owner?.path ? String(owner.path) : undefined,
            );
          ...
          classification = album.monitored
    ...
    progress?.(
      position + 1,
      total,
      result.recordingTitle ?? result.artistNames?.[0],
    );
  }
  return statuses;
}
