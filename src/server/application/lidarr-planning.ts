...
export function planLidarr(
  results: MusicBrainzResult[],
  existingArtists: Set<string>,
  existingGroups: Set<string>,
  allowedVariousArtists = new Set<string>(),
  representedLocally = new Set<number>(),
): LidarrPlan {
  const actions: LidarrPlanAction[] = [];
  const seen = new Set<string>();
  for (const [position, result] of results.entries()) {
    if (representedLocally.has(position)) {
      actions.push({ action: "skip", reason: "represented_locally" });
      continue;
    }
    const artist = result.primaryArtistId;
    const group = result.releaseGroupIds?.[0];
    if (!artist || !group) {
    ...
    if (
      ...
      !result.recordingIds?.some((id) => allowedVariousArtists.has(id))
    ) {
      actions.push({
        action: "skip",
        artistMbid: artist,
    ...
    if (!existingArtists.has(artist))
      actions.push({
        ...
        payload: { release_group_ids: [group] },
      });
    const payload = {
      requested_release_ids: result.releaseIds ?? [],
