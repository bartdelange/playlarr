...
export function lidarrPlanSummary(plan: LidarrPlan): LidarrPlanSummary {
  return {
    ...
    changes: plan.actions.filter((action) => mutatingActions.has(action.action))
      .length,
    artists: new Set(
      plan.actions.flatMap((action) =>
        action.artistMbid ? [action.artistMbid] : [],
      ),
    ).size,
    newArtists: plan.actions.filter(
      (action) => action.action === "create_artist",
    ).length,
    releases: new Set(
      plan.actions.flatMap((action) =>
    ...
    monitored: plan.actions.filter(
      (action) => action.action === "monitor_release",
    ).length,
    searches: plan.actions.filter((action) => action.action === "queue_search")
      .length,
    attention: plan.actions.filter((action) => action.action === "skip").length,
  };
}

export function lidarrPlanRows(
  entries: LidarrPlanEntry[],
  plan: LidarrPlan,
): LidarrPlanRow[] {
  ...
  for (const action of plan.actions) {
    if (action.releaseGroupId)
      append(actionsByGroup, action.releaseGroupId, action);
    else if (action.artistMbid)
      append(actionsByArtist, action.artistMbid, action);
  ...
  return entries.map((entry) => {
    ...
    const releases = (entry.result.releaseGroupIds ?? []).map((sourceGroup) => {
      const reuse = (reuseBySourceGroup.get(sourceGroup) ?? []).find((action) =>
        intersects(
          recordingIds,
          strings(action.payload, "requested_recording_ids"),
        ),
      );
      const lidarrGroup = reuse?.releaseGroupId ?? sourceGroup;
      const actions = (actionsByGroup.get(lidarrGroup) ?? []).filter(
        (action) => {
          if (
            action.artistMbid &&
            action.artistMbid !== entry.result.primaryArtistId
          )
            return false;
      ...
      const displayedMatch = actions.find((action) =>
        record(action.payload?.matched_track),
      );
      const releaseArtist = actions.find(
        (action) => action.artistName || action.artistMbid,
      );
      return {
        sourceGroup,
        lidarrGroup,
        ...
        artistName:
          releaseArtist?.artistName ?? entry.result.artistNames?.[0] ?? "",
        actions,
    ...
    const artistActions =
      actionsByArtist.get(entry.result.primaryArtistId ?? "") ?? [];
    const actions = unique([
      ...artistActions,
      ...releases.flatMap((release) => release.actions),
    ]);
    ...
    return {
      ...
      variousArtistsOverride: Boolean(
        entry.evidence.allow_various_artists_release,
      ),
    };
  });
}
...
function append(
  values: Map<string, LidarrPlanAction[]>,
  key: string,
