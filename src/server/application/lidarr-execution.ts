...
interface ApprovedPlanRepository {
  get(id: string): { status: string; plan: LidarrPlan };
  recordExecution(id: string, results: ExecutionResult[]): void;
}
interface LidarrExecutionTransport {
  ...
  createArtist?(mbid: string): Promise<Record<string, unknown>>;
  createAlbum?(
...
export async function executeApprovedPlan(
  repository: ApprovedPlanRepository,
  client: LidarrExecutionTransport,
  id: string,
  ...
  const album = async (group: string) => {
    if (!albums.has(group))
      albums.set(
        group,
  ...
  for (const action of current.plan.actions) {
    try {
      ...
      if (action.action === "monitor_artist") {
        ...
        if (artist.monitored === true && artist.monitorNewItems === "none")
          results.push({ outcome: "unchanged", details: "already_monitored" });
        else {
          artist.monitored = true;
          artist.monitorNewItems = "none";
          await client.request("PUT", `artist/${artist.id}`, artist);
          results.push({ outcome: "updated" });
        }
        continue;
      }
      const group = action.releaseGroupId ?? "";
      const existing = await album(group);
      if (action.action === "create_release") {
        if (existing)
          results.push(
            isVariousArtistsAlbum(existing) && !allowVarious
              ? { outcome: "skipped", details: "various_artists_album" }
              : { outcome: "unchanged", details: "release_exists" },
          );
        else {
          const artist = artists.get(artistMbid);
          if (!artist) throw new Error(`artist is unavailable: ${artistMbid}`);
          ...
          const created = await client.createAlbum(
            artist,
            group,
            stringPayload(action.payload, "requested_release_ids"),
          );
          if (!created)
            results.push({
              outcome: "skipped",
              details: "various_artists_album",
            });
          else {
            albums.set(group, created);
            changedReleases.add(group);
            results.push({ outcome: "created" });
          }
        }
        continue;
      ...
      if (isVariousArtistsAlbum(existing) && !allowVarious) {
        results.push({ outcome: "skipped", details: "various_artists_album" });
        continue;
      }
      if (action.action === "monitor_release") {
        const releaseChanged = pinSelectedRelease(
          existing,
          new Set(stringPayload(action.payload, "requested_release_ids")),
        );
        const monitorChanged = !existing.monitored;
        if (!releaseChanged && !monitorChanged)
          results.push({
            outcome: "unchanged",
            details: "already_monitored_and_release_selected",
          });
        else {
          existing.monitored = true;
          await client.request("PUT", `album/${existing.id}`, existing);
          changedReleases.add(group);
          results.push({ outcome: "updated" });
        }
        continue;
      ...
      if (!searchEnabled)
        results.push({
          outcome: "unchanged",
          details: "search_precondition_already_satisfied",
        });
      else {
        await client.request("POST", "command", {
          name: "AlbumSearch",
          albumIds: [existing.id],
        });
        results.push({ outcome: "queued" });
      }
    } catch (error) {
      results.push({
        outcome: "failed",
  ...
  repository.recordExecution(id, results);
  return results;
}
