...
export type LibraryAvailability = "downloaded" | "downloadable" | "not_downloadable" | "not_refreshed";
...
export function finalAvailabilityCounts(rows: FinalTableRow[]) {
  return {
    ...
    downloadable: rows.filter((row) => row.availability === "downloadable").length,
    not_downloadable: rows.filter((row) => row.availability === "not_downloadable").length,
  };
}

export function filterFinalRows(rows: FinalTableRow[], filter: "all" | LibraryAvailability) {
...
export function finalTableRows(
  ...
  const executed = new Map(execution.map((result) => [result.actionPosition, result]));
  ...
  return entries.map((entry) => {
    ...
    const executionNotes = relevantActions.flatMap(({ action, actionPosition }): FinalExecutionNote[] => {
    ...
    const matchedAction = relevantActions.find(({ action }) => record(action.payload?.matched_track))?.action;
    const matchedTrack = record(matchedAction?.payload?.matched_track);
    let availability = libraryAvailability(entry.libraryClassification, entry.libraryPath);
    if (executionNotes.some((note) => ["various_artists_album", "various_artists_skipped"].includes(note.reason ?? "")))
    ...
    return {
      ...
      lidarrMatch: matchedTrack
        ? {
            ...
            albumTitle: matchedAction?.albumTitle ?? entry.track.album ?? "Lidarr release",
...
export function libraryAvailability(classification?: string, path?: string): LibraryAvailability {
  if (!classification) return "not_refreshed";
  if (path || downloadedClassifications.has(classification)) return "downloaded";
  if (downloadableClassifications.has(classification)) return "downloadable";
  return "not_downloadable";
}

function actionMatchesEntry(action: LidarrPlanAction, result: MusicBrainzResult): boolean {
  if (action.artistMbid !== result.primaryArtistId) return false;
  const requested = strings(action.payload, "requested_recording_ids");
  if (requested.length && !intersects(requested, result.recordingIds ?? [])) return false;
  const sourceGroups = new Set(strings(action.payload, "mapped_release_group_ids"));
  if (action.releaseGroupId && result.releaseGroupIds?.includes(action.releaseGroupId))
