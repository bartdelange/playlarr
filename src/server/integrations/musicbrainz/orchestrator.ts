...
const setsEqual = (left: Set<string>, right: Set<string>) => left.size === right.size && [...left].every((value) => right.has(value));
...
export class MusicBrainzResolver {
  ...
  async resolve(track: SourceTrack): Promise<MusicBrainzResult> {
    ...
    const recordings = await this.recordings(`recording:"${searchTitle(track.title)}"${track.artists[0] ? ` AND artist:"${track.artists[0]}"` : ""}`, 10);
    ...
    const candidates = recordings.map((recording) => {
      const title = recording.title ?? ""; const terms = words(title);
      const overlap = sourceTerms.size && terms.size ? [...sourceTerms].filter((word) => terms.has(word)).length / new Set([...sourceTerms, ...terms]).size : 0;
      ...
      return { recording, terms, overlap, similarity, artistMatch, score: Number(recording.score ?? 0) + overlap * 100 + similarity * 25 };
    }).filter(({ recording, terms, overlap, similarity, artistMatch }) => artistMatch && overlap >= 0.3 && (similarity >= 0.55 || setsEqual(terms, sourceTerms)) && (!marked(track.title) || marked(recording.title ?? "")));
    const exact = candidates.filter(({ terms }) => setsEqual(terms, sourceTerms));
    const selected = (exact.length ? exact : candidates).sort((left, right) => versionPreference(right.recording.title ?? "") - versionPreference(left.recording.title ?? "") || right.score - left.score)[0];
  ...
  private async recordings(query: string, limit: number): Promise<MusicBrainzRecording[]> {
    const data = await this.client.get("recording", { query, inc: "artist-credits+releases+release-groups", limit: String(limit), fmt: "json" });
