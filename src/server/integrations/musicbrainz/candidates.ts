...
export function candidateFromRecording(
  ...
  const titleSimilarity = sequenceSimilarity(
  ...
  const releases = (recording.releases ?? []).map((release) => ({
    id: release.id ?? "",
    title: release.title ?? "",
    date: release.date ?? "",
  ...
  if (releaseGroupIds.length)
    result = {
      ...result,
      releaseIds: uniqueValues(releases.map((release) => release.id)),
      releaseGroupIds,
    };
  ...
  return {
    ...
    evidence: {
      ...
      candidateArtists: [...(result.artistNames ?? [])],
      versionPreference: versionPreference(candidateTitle),
    },
  };
}
...
function rank(candidate: Candidate): number[] {
  ...
  return [
    ...
    Number(sameBase),
    sameBase ? versionPreference(candidate.evidence.candidateTitle) : 0,
    candidate.score,
  ];
}
...
export function validateCandidate(candidate: Candidate): "valid" | "warning" {
  return validationWarnings(candidate).length ? "warning" : "valid";
}
