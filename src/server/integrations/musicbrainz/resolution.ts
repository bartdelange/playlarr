...
export function resultFromRecordings(
  recordings: MusicBrainzRecording[],
  resolvedVia: string,
  sourceAlbum: string,
): MusicBrainzResult | undefined {
  const recordingIds: string[] = [];
  const releaseIds: string[] = [];
  const releaseGroupIds: string[] = [];
  const artistIds: string[] = [];
  let primaryArtistId: string | undefined;
  const releases: (MusicBrainzRelease & {
    id?: string;
    "release-group"?: { id?: string };
  })[] = [];
  for (const recording of recordings) {
    if (recording.id) recordingIds.push(recording.id);
    const credits = recording["artist-credit"] ?? [];
    if (!primaryArtistId) primaryArtistId = credits[0]?.artist?.id;
    credits.forEach((credit) => {
      if (credit.artist?.id) artistIds.push(credit.artist.id);
    });
    recording.releases?.forEach((release) => {
      if (release["release-group"]?.id) releases.push(release);
    });
  }
  ...
  if (releases.length) {
    ...
    if (selectedGroup) {
      releaseGroupIds.push(selectedGroup);
      releases
        .filter((release) => release["release-group"]?.id === selectedGroup)
        .forEach((release) => {
          if (release.id) releaseIds.push(release.id);
        });
    }
  }
  ...
  return {
    ...
    artistIds: uniqueValues(artistIds),
    primaryArtistId,
  };
}
