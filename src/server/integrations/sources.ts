...
import { FileSpotifyPendingAuthorizationStore, FileSpotifyTokenStore, SpotifyAuthenticator } from "./spotify/auth";
...
export function spotifyProvider(config: AppConfig, settings: SettingsRepository) {
  ...
  const auth = new SpotifyAuthenticator(
    ...
    new FileSpotifyPendingAuthorizationStore(`${config.spotify.tokenCache}.pending`),
  );
  return { auth, source: new SpotifySource(auth) };
}

export function tidalProvider(config: AppConfig) {
  const auth = new TidalAuthenticator(new FileTidalSessionStore(config.tidal.sessionFile));
