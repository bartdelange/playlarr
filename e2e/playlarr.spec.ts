...
test("login, dashboard, playlist detail, review, revisions, jobs, and settings use the real app", async ({ page }) => {
  await login(page);
  ...
  await expect(page.getByRole("button", { name: "Delete import" })).toBeVisible();
  await expect(page.getByRole("navigation", { name: "Import navigation" })).toHaveCount(0);
  await expect(page.getByRole("link", { name: "Reuse mappings" })).toBeVisible();
  ...
  await expect(page.getByRole("button", { name: "Refresh monitored & downloaded" })).toBeVisible();
  ...
  await expect(page.getByRole("heading", { name: "Fixture Playlist" })).toBeVisible();
  await expect(page.getByRole("heading", { name: /Before · 1 tracks/ })).toBeVisible();
  ...
  await expect(page.getByRole("heading", { name: "Local additions" })).toBeVisible();
  await page.goto(`/imports/${fixtureImportId}`);
  ...
  await expect(page.getByRole("heading", { name: "Review changes to Fixture Playlist" })).toBeVisible();
  ...
  await expect(page.locator('select[name="lidarr_quality_profile_id"]')).toHaveValue("1");
  await expect(page.locator('select[name="lidarr_metadata_profile_id"]')).toHaveValue("1");
  await expect(page.getByText("Services", { exact: true })).toBeVisible();
  await page.getByText("Data Settings", { exact: true }).click();
  await expect(page.getByRole("heading", { name: "Playlist paths" })).toBeVisible();
  await page.goto("/imports/new");
  await expect(page.getByRole("heading", { name: "Choose a source" })).toBeVisible();
  await expect(page.getByRole("button", { name: "Choose Spotify" })).toBeVisible();
  await expect(page.getByRole("button", { name: "Choose TIDAL" })).toBeVisible();
});
...
test("job progress and logout enforce session state", async ({ page }) => {
  ...
  await page
    ...
    .first()
    .click();
  await expect(page.locator("main .job-completed").first()).toHaveText("completed");
...
test("manual review session exposes navigation and advances after a decision", async ({ page }) => {
  await login(page);
  ...
  await expect(page.getByRole("button", { name: "Validate MBID" })).toBeVisible();
  await expect(page.getByRole("button", { name: "Search" })).toBeVisible();
  await page.getByRole("button", { name: "Skip track and continue" }).click();
  await expect(page.getByRole("heading", { name: "Review Second" })).toBeVisible();
